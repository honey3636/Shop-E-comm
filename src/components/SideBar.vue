<template>
    <div class="px-0 m-0  fixed top-[65px] ">
        <div  class="pt-2 m-0 ">
            ...
            <ul v-if="selectedAccount==='switchToBuying'" class="flex flex-col gap-2 w-full">
                
                <li  v-for="(item, index) in sidebarMenuItems " :key="index" class=" p-2 w-full hover:bg-gray-200">
                    <router-link :to="item.path" class="   text-muted a-Link ps-3" active-class="active">
                        <img class="h-5 inline" :src="item.image" />
                        <span class="ms-2 hidden sm:inline ">{{ item.name }}</span>
                    </router-link>
                </li>
            </ul>
            <ul v-if="selectedAccount==='switchToSeller'" class="flex flex-col gap-2 w-full">
                
                <li  v-for="(item, index) in itemsForSeller " :key="index" class="p-2 w-full hover:bg-gray-200">
                    <router-link :to="item.path" class="   text-muted a-Link ps-3" active-class="active">
                        <img class="h-5 inline" :src="item.image" />
                        <span class="ms-2 hidden sm:inline">{{ item.name }}</span>
                    </router-link>
